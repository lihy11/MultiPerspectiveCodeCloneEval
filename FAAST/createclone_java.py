...
def createast(cross):
    ...
    paths = []
    alltokens = []
    dirname = '../dataset/gcj/code0/'
    if cross is not None:
        dirname = f'./{cross}/'
    err = []
    for file in os.listdir(dirname):
        try:
            programfile = open(os.path.join(dirname, file), encoding='utf-8')
...
def creategmndata(id, treedict, vocablen, vocabdict, device):
    indexdir = './gcj/'
    if id == 'comp0':
        trainfile = open(indexdir + 'gcj_sample.txt')
        testfile = open(indexdir + 'gcj_old.txt')
        ...
        trainfile = open(indexdir + 'gcj_old.txt')
        testfile = open(indexdir + 'gcj_sample.txt')
        ...
        trainfile = open(indexdir + f'train_{id}.txt')
        validfile = open(indexdir + f'val_{id}.txt')
        testfile = open(indexdir + f'test_{id}.txt')
